...
if(! defined('FILE_CACHE_DIRECTORY') )    define ('FILE_CACHE_DIRECTORY', '../view/upload/images/cache');       // Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
...
if(! defined('LOCAL_FILE_BASE_DIRECTORY') )      define ('LOCAL_FILE_BASE_DIRECTORY', false);
...
class timthumb {
  ...
  protected $images_param = array();
  
  public static function start($images_param) {
    $tim = new timthumb($images_param);
    $tim->handleErrors();
    $tim->securityChecks();
  ...
  public function __construct($images_param) {
    global $ALLOWED_SITES;
    $this->images_param = $images_param;
    ...
    if($this->isURL) {
      ...
      asort($arr);
      $this->cachefile = $this->cacheDirectory . '/' . FILE_CACHE_PREFIX . $cachePrefix . md5($this->salt . implode('', $this->images_param) . $this->fileCacheVersion) . FILE_CACHE_SUFFIX;
    } else {
      ...
      $this->cachefile = $this->cacheDirectory . '/' . FILE_CACHE_PREFIX . $cachePrefix . md5($this->salt . $this->localImageMTime . implode('', $this->images_param) . $this->fileCacheVersion . $this->src) . FILE_CACHE_SUFFIX;
    }
  ...
  protected function processImageAndWriteToCache($localImage){ 
    ...
    if(flock($fh, LOCK_EX)){
      if (is_file($this->cachefile)) unlink($this->cachefile); //rename generally overwrites, but doing this in case of platform specific quirks. File might not exist yet.
      ...
      fclose($fh);
      if (is_file($lockFile)) unlink($lockFile);
      if (is_file($tempfile4)) unlink($tempfile4);
  ...
  protected function getLocalImagePath($src) {
    if (is_file($this->realpath($src))) 
      return $this->realpath($src);
    return false;
  ...
  protected function param($property, $default = ''){
    if (isset ($this->images_param[$property])) {
      return $this->images_param[$property];
    } else {
      return $default;
    }
  }
  ...
  protected function getIP(){
    $rem = (isset($_SERVER["REMOTE_ADDR"])) ? $_SERVER["REMOTE_ADDR"] : '';
    $ff = (isset($_SERVER["HTTP_X_FORWARDED_FOR"])) ? $_SERVER["HTTP_X_FORWARDED_FOR"] : '';
    $ci = (isset($_SERVER["HTTP_CLIENT_IP"])) ? $_SERVER["HTTP_CLIENT_IP"] : '';
  ...
  protected function debug($level, $msg){
    if(DEBUG_ON && $level <= DEBUG_LEVEL){
      ...
      print_r("TimThumb Debug line " . __LINE__ . " [$execTime : $tick]: $msg");
      echo "\n\r";
    }
  }
