...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true) {
        $this->options = array(
            'script_url' => 'http://'.Core::inst()->script_url.'upload/jquery_file_upload/',
            'upload_dir' => '../view/upload/images/',
            'upload_url' => 'http://'.Core::inst()->script_url.'view/upload/images/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    protected function get_file_objects($iteration_method = 'get_file_object') {
        ...
        $files = array();
        $gallery = Core::inst()->album->get_gallery((is_numeric(end($arr))) ? end($arr) : prev($arr));
        foreach ($gallery['photo'] AS $photo)
          $files[] = $photo['album_photo_name'];
    ...
    protected function trim_file_name($name, $type, $index, $content_range) {
        ...
        $file_name = md5($file_name . time()) . $matches[0];
        
        // Add missing file extension for known image types:
        if (strpos($file_name, '.') === false &&
            preg_match('/^image\/(gif|jpe?g|png)/', $type, $matches)) {
            $file_name .= md5($file_name . date()) . '.' . $matches[1];
        }     
    ...
    public function delete($print_response = true) {
        ...
        $file_path = $this->get_upload_path($file_name);
        ///////////
        $_res = Core::inst()->album->del_photo($file_name);
        if (isset($_res['error']) && $_res['error']) return $this->generate_response($info, $print_response);
        ///////////
