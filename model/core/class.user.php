  ...
  class di_user {
    ...
    function __construct() { 
      ...
      if ($this->is_loggedin()) {
        ...
        $this->user_id = $_SESSION['di_id'];
      }
    }
    
    function login($login, $password) {
      if (!isset($login{0}) || !isset($password{3})) return array(false, 1); // Пароль или ник не коректны
      
      $tel_array = $this->get_phone($login);
      $str_tel = $tel_array[0].$tel_array[1].$tel_array[2];
      
      $password = md5(md5($password).Core::inst()->salt_bd);
      Core::inst()->db->query('SELECT `user_id`, `user_is_confirmed`, `user_hash` FROM `di_user` WHERE `user_tel` = ? AND `user_password` = ?;', $str_tel, $password);
      if (Core::inst()->db->num_rows() != 1) return array(false, 2); // Ник или пароль введены не верно
      if (Core::inst()->db->result(false, 0, 'user_is_confirmed') != '1') return array(false, 3); // Аккаунт не подтверждён
      
      $this->user_id = Core::inst()->db->result(false, 0, 'user_id');
      $hash = Core::inst()->db->result(false, 0, 'user_hash');
      
      setcookie('di_hash', md5(sha1(Core::inst()->salt_au).sha1(md5($hash))), (time()+2592000), '/', '', 0);
      if (!session_id()) session_start();
      $_SESSION['di_id'] = $this->user_id;
      $_SESSION['di_hash'] = md5(sha1(Core::inst()->salt_session).sha1(md5($hash)));
      
      return array(true, '');
    }
    
    function is_loggedin() {
      if (isset($this->logged_in)) return $this->logged_in;
      if (!session_id()) session_start();
      if (!isset($_COOKIE['di_hash']) || !isset($_SESSION['di_id']) || !isset($_SESSION['di_hash'])) return false;

      Core::inst()->db->query('SELECT `user_hash` FROM `di_user` WHERE `user_id` = ?;', $_SESSION['di_id']);
      if (Core::inst()->db->num_rows() != 1) return false;
      
      $hash = Core::inst()->db->result(false, 0, 'user_hash');
      
      if ($_COOKIE['di_hash'] != md5(sha1(Core::inst()->salt_au).sha1(md5($hash)))) return false;
      if ($_SESSION['di_hash'] != md5(sha1(Core::inst()->salt_session).sha1(md5($hash)))) return false;
    ...
    function logout() {
      if (!session_id()) session_start();
      setcookie('di_hash', '', (time()+2592000), '/', '', 0);
      unset($_SESSION['di_id']);
      unset($_SESSION['di_hash']);
      unset($this->logged_in);
    }
    ...
    function logout_all() {
      ...
      $hash = '';
      while (strlen($hash) < 255) $hash .= $chars[mt_rand(0,  strlen($chars)-1)];
      
      Core::inst()->db->query('UPDATE `di_user` SET `user_hash` = ? WHERE `user_id` = ?;', $hash, $_SESSION['di_id']);
      $this->logout();
    }
    
    function valid_login_and_password($login, $password) {
      if (!isset($login{0}) || !isset($password{3})) return false;
      ...
      Core::inst()->db->query('SELECT `user_id` FROM `di_user` WHERE `user_tel` = ? AND `user_password` = ? AND `user_is_confirmed` = ?;', $str_tel, $password, 1);
      if (Core::inst()->db->num_rows() != 1) return false;
    ...
    function get_info() {
      ...
      if (count($this->info) > 0) return $this->info;
      
      $result = Core::inst()->db->query('SELECT `usermeta_key`, `usermeta_val` FROM `di_usermeta` WHERE `usermeta_user_id` = ?;', $_SESSION['di_id']);
      while($data = Core::inst()->db->fetch_assoc($result)) 
        $return_info[$data['usermeta_key']] = $this->filter_xss_and_return_one_line($data['usermeta_val']);
      return $this->info = $return_info;
    }
    
    function get_id() {
      if (!$this->is_loggedin()) return false;
      return $_SESSION['di_id'];
    }
    
    function user_info_refresh($name, $screen_name, $email, $url, $description) {
      if (!$this->is_loggedin()) return array(false, 'Ошибка!');
      // Имя пользователя
      if (!isset($name)) $name = '';
      else {
        if (strlen($name) > 20) return array(false, 'Имя пользователя содержит содержит слишком много символов.');
        if (!$this->name_valid($name)) return array(false, 'Имя пользователя содержит недопустимые символы.');
      }
      Core::inst()->db->query('INSERT INTO `di_usermeta` (`usermeta_user_id`, `usermeta_key`, `usermeta_val`) VALUES(?, ?, ?) ON DUPLICATE KEY UPDATE `usermeta_val` = VALUES(`usermeta_val`);', $this->user_id, 'name', $name);
      // Логин пользователя
      if (!isset($screen_name) || strlen($screen_name) == 0) $screen_name = '';
      else {
        if (strlen($screen_name) > 15) return array(false, 'Логин пользователя содержит содержит слишком много символов.');
        if (!$this->url_name_valid($screen_name)) return array(false, 'Логин пользователя содержит недопустимые символы.');
        if ($this->reserved_usernames($screen_name)) return array(false, 'Логин пользователя уже используется.');
        
        Core::inst()->db->query('SELECT * FROM `di_usermeta` WHERE `usermeta_key` = ? AND `usermeta_val` = ? AND `usermeta_user_id` != ?;', 'screen_name', $screen_name, $this->get_id());
        if (Core::inst()->db->num_rows() > 0) return array(false, 'Логин пользователя уже используется.');
      }
      Core::inst()->db->query('INSERT INTO `di_usermeta` (`usermeta_user_id`, `usermeta_key`, `usermeta_val`) VALUES(?, ?, ?) ON DUPLICATE KEY UPDATE `usermeta_val` = VALUES(`usermeta_val`);', $this->user_id, 'screen_name', $screen_name);
      // Электронная почта пользователя
      if (!isset($email) || strlen($email) == 0) $email = '';
      else {
        if (strlen($email) > 200) return array(false, 'Электронная почта пользователя содержит содержит слишком много символов.');
        if (!$this->email_valid($email)) return array(false, 'Электронная почта пользователя не поддерживается.');
      }
      Core::inst()->db->query('INSERT INTO `di_usermeta` (`usermeta_user_id`, `usermeta_key`, `usermeta_val`) VALUES(?, ?, ?) ON DUPLICATE KEY UPDATE `usermeta_val` = VALUES(`usermeta_val`);', $this->user_id, 'email', $email);
      // Сайт пользователя
      if (!isset($url) || strlen($url) == 0) $url = '';
      else {
        if (strlen($url) > 200) return array(false, 'Сайт пользователя содержит содержит слишком много символов.');
        if (!$this->url_valid($url)) return array(false, 'Сайт пользователя содержит недопустимые символы.');
      }
      Core::inst()->db->query('INSERT INTO `di_usermeta` (`usermeta_user_id`, `usermeta_key`, `usermeta_val`) VALUES(?, ?, ?) ON DUPLICATE KEY UPDATE `usermeta_val` = VALUES(`usermeta_val`);', $this->user_id, 'url', $url);
      // Описание пользователя
      if (!isset($description) || strlen($description) == 0) $description = '';
      else {
        if (strlen($description) > 255) return array(false, 'Описание пользователя содержит содержит слишком много символов.');
      }
      Core::inst()->db->query('INSERT INTO `di_usermeta` (`usermeta_user_id`, `usermeta_key`, `usermeta_val`) VALUES(?, ?, ?) ON DUPLICATE KEY UPDATE `usermeta_val` = VALUES(`usermeta_val`);', $this->user_id, 'description', $description);
      return array(true, 'Ваши настройки сохранены.');
    }
    
    private function url_name_valid($name) {
      $span_str = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_';
      if (strspn($name, $span_str) == 0) return false;
      if (strspn($name, $span_str) != strlen($name)) return false;
      return true;
    }

    private function name_valid($name) {
      return preg_match('/^[\x7F-\xffA-Za-z0-9\Q ,_-:`\E]*$/', $name);
    }

    private function email_valid($email) {
      return preg_match('/^(?:[a-z0-9]+(?:[-_.]?[a-z0-9]+)?@[a-z0-9]+(?:\.?[a-z0-9]+)?\.[a-z]{2,5})$/is', $email);
    }

    private function url_valid($url) {
       return preg_match("~^(?:(?:https?|ftp|telnet)://(?:[a-z0-9_-]{1,32}(?::[a-z0-9_-]{1,32})?@)?)?(?:(?:[a-z0-9-]{1,128}\.)+(?:com|net|org|mil|edu|arpa|gov|biz|info|aero|inc|name|[a-z]{2})|(?!0)(?:(?!0[^.]|255)[0-9]{1,3}\.){3}(?!0|255)[0-9]{1,3})(?:/[a-z0-9.,_@%&?+=\~/-]*)?(?:#[^ '\"&<>]*)?$~i",$url);
    }
    
    private function filter_xss($text) {
      return htmlentities(strip_tags($text), ENT_QUOTES, 'UTF-8');
    }
    
    private function filter_xss_and_return_one_line($text) {
