<?php
  if (is_file('data/config.bd.php')) exit(0);
  ...
  $warnings = array();
  /* Test PHP */
  if (version_compare(PHP_VERSION, '5.3.0', '<')) $errors[] = 'Необходим PHP 5.3 или выше';
  if ((int)preg_replace('/[^0-9\s]/', '', ini_get('memory_limit')) < 128) 
    if (!ini_set('memory_limit', '128M')) $warnings[] = 'Желателен memory_limit = 128M или выше';
  if ((int)preg_replace('/[^0-9\s]/', '', ini_get('upload_max_filesize')) < 20) 
    if (!ini_set('upload_max_filesize', '20M')) $warnings[] = 'Желателен upload_max_filesize = 20M или выше';
  if (substr(sprintf('%o', fileperms('error.log')), -4) != '666') $errors[] = 'Файл "model/error.log" требует права 0666';
  if (substr(sprintf('%o', fileperms('../view/upload/images/cache/timthumb_cacheLastCleanTime.touch')), -4) != '0666') $errors[] = 'Файл "view/upload/images/cache/timthumb_cacheLastCleanTime.touch" требует права 0666';
  if (substr(sprintf('%o', fileperms('../view/upload/images/cache/index.html')), -4) != '0666') $errors[] = 'Файл "view/upload/images/cache/index.html" требует права 0666';
  foreach(array(
    '../view/cache/',
    '../view/templates_c/',
    '../view/upload/images/',
  ...
  if (isset($_POST['command']) && $_POST['command'] == 'setup') {
    /* Test Data */
    if (!isset($site_name{1})) $errors[] = 'Название сайта слишком коротко';
    /* Test user Data */
    $array_tel = get_phone($user_login);
    if (!isset($array_tel[0]) || !in_array($array_tel[0], array(7, 380))) $errors[] = 'Номер телефона этой страны не поддерживается';
    if (!isset($user_pass{3})) $errors[] = 'Пароль слишком краток';
    
    if (!mysql_connect($db_host, $db_user, $db_pass)) $errors[] = 'Не верны данные доступа к Базе Данных';
    if (!mysql_select_db($db_name)) $errors[] = 'Не найдена База Данных';
    
    preg_match('@[0-9]+\.[0-9]+@', mysql_get_server_info(), $version_mysql);
    if (version_compare($version_mysql[0], '5.3', '<')) $errors[] = 'Необходим Mysql 5.3 или выше';
    
    if (count($errors) == 0) {
      $chars = '1234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ ^:;.,`&$}~!?#[]{}()_/|*<>=+-@%';
      $salt_bd = '';
      while (strlen($salt_bd) < mt_rand(0, 255)) $salt_bd .= $chars[mt_rand(0,  strlen($chars)-1)];
      $salt_au = '';
      while (strlen($salt_au) < mt_rand(0, 255)) $salt_au .= $chars[mt_rand(0,  strlen($chars)-1)];
      $salt_session = '';
      while (strlen($salt_session) < mt_rand(0, 255)) $salt_session .= $chars[mt_rand(0,  strlen($chars)-1)];
      $img_password = '';
      while (strlen($img_password) < mt_rand(0, 255)) $img_password .= $chars[mt_rand(0,  strlen($chars)-1)];
      $img_prefix = '';
      while (strlen($img_prefix) < mt_rand(0, 255)) $img_prefix .= $chars[mt_rand(0,  strlen($chars)-1)];
 
      $fd = fopen('data/config.bd.php', 'w+');
      fwrite($fd, '<'."?php\n");
      fwrite($fd, '  Core::inst()->bdhost = \''.$db_host.'\';'."\n");
      fwrite($fd, '  Core::inst()->bdlogin = \''.$db_user.'\';'."\n");
      fwrite($fd, '  Core::inst()->bdpassword = \''.str_replace("'", "\'", $db_pass).'\';'."\n");
      ...
      fwrite($fd, '  Core::inst()->salt_bd = \''.$salt_bd.'\';'."\n");
      fwrite($fd, '  Core::inst()->salt_au = \''.$salt_au.'\';'."\n");
      fwrite($fd, '  Core::inst()->salt_session = \''.$salt_session.'\';'."\n");
      fwrite($fd, '  Core::inst()->img_password = \''.$img_password.'\';'."\n");
      fwrite($fd, '  Core::inst()->img_prefix = \''.$img_prefix.'\';'."\n");
      fwrite($fd, '?'.'>');
      fclose($fd);
      ...
      ');
      $query = mysql_query('
        CREATE TABLE IF NOT EXISTS `di.usermeta` (
          `usermeta_user_id` int(15) NOT NULL DEFAULT \'0\',
          `usermeta_key` varchar(255) CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL,
          `usermeta_val` longtext COLLATE utf8mb4_unicode_ci NOT NULL,
        PRIMARY KEY (`usermeta_user_id`,`usermeta_key`)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
      ');
      $query = mysql_query('
        CREATE TABLE IF NOT EXISTS `di.album_gallery` (
          `album_gallery_id` int(15) NOT NULL AUTO_INCREMENT,
          `album_gallery_user_id` int(15) DEFAULT NULL,
          `album_gallery_name` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL,
          `album_gallery_created_ip` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL,
          `album_gallery_created_date` int(11) NOT NULL,
          `album_gallery_visible` tinyint(4) NOT NULL DEFAULT \'1\',
          `album_gallery_priority` int(11) NOT NULL DEFAULT \'1\',
        PRIMARY KEY (`album_gallery_id`),
        KEY `album_gallery_user_id` (`album_gallery_user_id`)
        ) ENGINE=InnoDB  DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
      ');
      $query = mysql_query('
        CREATE TABLE IF NOT EXISTS `di.album_photo` (
          `album_photo_id` int(15) NOT NULL AUTO_INCREMENT,
          `album_photo_gallery_id` int(15) DEFAULT NULL,
          `album_photo_name` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL,
          `album_photo_created_ip` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL,
          `album_photo_created_date` int(11) NOT NULL,
        PRIMARY KEY (`album_photo_id`),
        KEY `album_photo_gallery_id` (`album_photo_gallery_id`)
        ) ENGINE=InnoDB  DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
      ');
      $query = mysql_query('ALTER TABLE `di.album_gallery` DD CONSTRAINT `di@002ealbum_gallery_ibfk_1` FOREIGN KEY (`album_gallery_user_id`) REFERENCES `di_user` (`user_id`) ON DELETE SET NULL ON UPDATE CASCADE;');
      $query = mysql_query('ALTER TABLE `di.album_photo` ADD CONSTRAINT `di@002ealbum_photo_ibfk_1` FOREIGN KEY (`album_photo_gallery_id`) REFERENCES `di.album_gallery` (`album_gallery_id`) ON DELETE SET NULL ON UPDATE CASCADE;');
      $query = mysql_query('
        INSERT INTO `di_user` (
          `user_tel`, `user_password`, `user_remote_addr`, `user_is_confirmed`, `user_date_created`)
        VALUES (
          \''.mysql_real_escape_string($array_tel[0].$array_tel[1].$array_tel[2]).'\', \''.mysql_real_escape_string(md5(md5($user_pass).$salt_bd)).'\', \''.mysql_real_escape_string($_SERVER['REMOTE_ADDR']).'\', 1, '.time().');
      ');
...
<!--[if gt IE 8]><!--> <html class='no-js'> <!--<![endif]-->
  <head>
    ...
    <link rel='stylesheet' href='http://<?php echo $_SERVER['SERVER_NAME'] . preg_replace('/^(.*?)model\/index\.php$/is', '$1', $_SERVER['SCRIPT_NAME']); ?>view/include/css/metro_style/modern.css'>
    <link rel='stylesheet' href='http://<?php echo $_SERVER['SERVER_NAME'] . preg_replace('/^(.*?)model\/index\.php$/is', '$1', $_SERVER['SCRIPT_NAME']); ?>view/include/css/metro_style/modern-responsive.css'>
  </head>
  <body>
    <div class='page'>
      <div class='page-header'>
        <div class='page-header-content'><h1>Тебя привествует мастер установки</h1></div>
      </div>
      <div class='page-region'>
        <div class='page-region-content' style='padding-left: 20px;'>
          <?php foreach ($errors as $key => $error) { ?>
          <div class='error-bar span10' style='position: relative;'><?php echo $error; ?></div>
          <?php } ?>
          <?php foreach ($warnings as $key => $warning) { ?>
          <div class='warning-bar span10' style='position: relative;'><?php echo $warning; ?></div>
          <?php } ?>
      
          <form action=<?php echo $script_url; ?> method='post' class='span5'>
            <input type='hidden' name='command' value='setup'>
            <h2>Параметры сайта</h2>
            <div class='input-control text'><input type='text' name='site_name' value='<?php echo $site_name; ?>' placeholder='название сайта' /></div>
            <div class='input-control text'><input type='text' name='site_description' value='<?php echo $site_description; ?>' placeholder='описание сайта' /></div>
            <h2>Установка Базы данных</h2>
            <div class='input-control text'><input type='text' name='db_host' value='<?php echo $db_host; ?>' placeholder='хост базы данных' /></div>
            <div class='input-control text'><input type='text' name='db_user' value='<?php echo $db_user; ?>' placeholder='имя пользователя базы данных' /></div>
            <div class='input-control text'><input type='text' name='db_pass' value='<?php echo $db_pass; ?>' placeholder='пароль пользователя базы данных' /></div>
            <div class='input-control text'><input type='text' name='db_name' value='<?php echo $db_name; ?>' placeholder='название базы данных' /></div>
            <h2>Параметры пользователя</h2>
            <div class='input-control text'><input type='text' name='user_login' value='<?php echo $user_login; ?>' placeholder='номер телефона' /></div>
            <div class='input-control text'><input type='text' name='user_pass' value='<?php echo $user_pass; ?>' placeholder='пароль' /></div>
            <input type='submit' name='button' value='далее' />
          </form>
        </div>
      </div>
    </div>
...
<?php
    function get_phone($phone = '', $convert = true, $trim = true) {
      $phoneCodes=Array(
       '9162'=>Array(
              'name'=>'Cocos',
              'cityCodeLength'=>0,
      ...
      foreach ($phoneCodes as $countryCode=>$data)
      {
          ...
          if (substr($phone, 0, $codeLen)==$countryCode)
          {
              ...
              return array($countryCode, $cityCode, $phone);
          }
      }
